        ...
        constructor(options: any) {
          ...
          this.wasmInstance = new WebAssembly.Instance(module, {
            wasi_snapshot_preview1: {
              ...
              environ_sizes_get: (environCount: number, environSize: number) => 0,
              args_get: (args: number, argsBuf: number) => 0,
              args_sizes_get: (argc: number, argv_buf_size: number) => 0,
              fd_close: (fd: number) => 0,
              fd_seek: (fd: number, offset_low: number, offset_high: number, whence: number, newOffset: number) => 0,
