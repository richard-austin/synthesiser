...
export enum envelopePhaseX {inactive, attack, decay, sustain, release, retrigger, legato }

export class OscillatorArrayX {
  ...
  async start(): Promise<void> {
    function worklet() {
      ...
      class Envelope {
        ...
        public envelopePhaseX: envelopePhaseX = envelopePhaseX.inactive;
        ...
        public advanceToSustain() {
          ...
          if (this.keyDown) {
            if (!envData.legato) {
              if (this.envelopePhaseX !== envelopePhaseX.attack) {
                this.inUse = true;
                const attackTarget = envData.velocitySensitive ? vel : 1;
                this.setEnvelopePhaseTiming(attackTarget, envData.attack);
                this.envelopePhaseX = envelopePhaseX.attack;
              } else if (this.envelopePhaseX === envelopePhaseX.attack) {
                this.level = this.exponentialRampToValueAtTime();
                if (this.targetReached) {
                  this.envelopePhaseX = envelopePhaseX.decay;
                  this.setEnvelopePhaseTiming(envData.sustainLevel, envData.decay);
                }
              } else if (this.envelopePhaseX === envelopePhaseX.decay) {
                this.level = this.exponentialRampToValueAtTime();
                if (this.targetReached) {
                  this.envelopePhaseX = envelopePhaseX.sustain;
                }
              }
            } else {
              if (this.envelopePhaseX !== envelopePhaseX.attack) {
        ...
        public advanceToZero() {
          ...
          if (!this.keyDown) {
            if (!envData.legato) {
              if (this.envelopePhaseX !== envelopePhaseX.release) {
                this.envelopePhaseX = envelopePhaseX.release;
                this.setEnvelopePhaseTiming(this.lowestLevel, envData.release);
              } else if (this.envelopePhaseX === envelopePhaseX.release) {
                this.level = this.exponentialRampToValueAtTime();
                if (this.targetReached) {
                  this.level = this.lowestLevel;
                  this.inUse = false;
                  this.envelopePhaseX = envelopePhaseX.inactive;
                }
              }
            } else {
              if (this.envelopePhaseX === envelopePhaseX.attack || this.envelopePhaseX === envelopePhaseX.decay) {
                this.setEnvelopePhaseTiming(0, envData.decay); /* Value doesn't matter here */
                this.envelopePhaseX = envelopePhaseX.sustain;
              } else if (this.envelopePhaseX === envelopePhaseX.sustain) {
      ...
      enum envelopePhaseX {inactive, attack, decay, sustain, release, retrigger, legato }
      ...
      class OscillatorData {
        ...
        // @ts-ignore
        public lpf = new LadderFilter4Pole(sampleRate, 1000, 3, 6);
      ...
      registerProcessor('oscillator', class Processor extends AudioWorkletProcessor {
          ...
          for (let i = 0; i < op.length; i++) {
            for (let b = 0; b < this.numberOfBanks; ++b) {
              ...
              for (let osc = 0; osc < this.oscillatorsPerBank; osc++) {
                ...
                let f = od.frequency;
                if(env.inUse )
                   od.lpf.setCutoff(od.frequency* env.level * 15);
                ...
                if(env.inUse)
                  outputChannel[i] += ampEnvelope * od.lpf.process(signal);
              }
            }
          }
        ...
        public keyDown(key: number, velocity: number) {
          ...
          if (i !== -1) {
            this.oscData.forEach((osc, bank) => {  /* forEach bank */
              ...
              if (od.env.envelopePhaseX !== envelopePhaseX.retrigger)
        ...
        private setEnvelope(bank: number, phase: envelopePhaseX, value: number) {
          const env = this.bankData[bank].envelopeData;
          switch (phase) {
            case envelopePhaseX.attack:
              env.attack = value;
              break;
            case envelopePhaseX.decay:
              env.decay = value;
              break;
            case envelopePhaseX.sustain:
              env.sustainLevel = value;
              break;
            case envelopePhaseX.release:
              env.release = value;
              break;
            case envelopePhaseX.legato:
              env.legato = value > 0;
              break;
            default:
              break;
        ...
        private getVacantOscillator(key: number): number {
          ...
          if (od) {
            ...
            bd.forEach((b, bankIndex) => {
              ...
              this.oscData[bankIndex][i].env.envelopePhaseX = envelopePhaseX.retrigger;
              // @ts-ignore
    ...
    this.node = new AudioWorkletNode(this.context, 'oscillator', {
      ...
      processorOptions: {
        ...
        waveTableSize: OscillatorArrayX.waveTableSize,
        startFx: OscillatorArrayX.startFx
      }
    });
  ...
  public static createPeriodicWave(audioCtx: AudioContext, real: number[], imag: number[], constraints: {
    ...
    this.lastReal = real;
    this.lastImag = imag;
    const refFreq = audioCtx.sampleRate / OscillatorArrayX.waveTableSize;
    const sampleRate = audioCtx.sampleRate;
    const retVal = [];
    const root2 = Math.pow(2, 1 / 2);
    for (let fx = this.startFx; fx < sampleRate / 2; fx *= root2) {
      const olac = new OfflineAudioContext(1, OscillatorArrayX.waveTableSize, sampleRate);
  ...
  public envelope(bank: number, phase: envelopePhaseX, value: number) {
