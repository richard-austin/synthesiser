    ...
    function worklet() {
      ...
      class Envelope {
        ...
        public advanceToSustain() {
          ...
          if (this.keyDown) {
            if (!envData.legato) {
              if (this.envelopePhase !== envelopePhase.attack) {
                this.inUse = true;
        ...
        public advanceToZero() {
          ...
          if (!this.keyDown) {
            if (!envData.legato) {
              ...
              if (this.envelopePhase === envelopePhase.attack || this.envelopePhase === envelopePhase.decay) {
      ...
      class OscillatorData {
        ...
        // @ts-ignore
        public lpf = new LadderFilter4Pole(sampleRate, 1000, 5);
          ...
          for (let i = 0; i < op.length; i++) {
            for (let b = 0; b < this.numberOfBanks; ++b) {
              ...
              for (let osc = 0; osc < this.oscillatorsPerBank; osc++) {
                ...
                let f = od.frequency;
                if(od.lpf.getCutoff() != od.frequency)
                   od.lpf.setCutoff(od.frequency);
